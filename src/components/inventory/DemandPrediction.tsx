...
export function DemandPrediction({ selectedProject }: DemandPredictionProps) {
  const [selectedPredictionProject, setSelectedPredictionProject] =
    useState("");
  ...
  useEffect(() => {
    const fetchRequestsData = async () => {
      try {
        ...
        const data = await consumeKafkaTopic(
          "scm_requests",
          undefined,
          5000,
          "earliest"
        ...
        const projects = Array.from(
          new Set(
            normalized.map((it: any) => it.project_display).filter(Boolean)
          )
        ).sort();
        setProjectOptions(
          projects.length ? projects : ["No projects available"]
        );

        // Extract global unique items
        const items = Array.from(
          new Set(normalized.map((it: any) => it.item_name).filter(Boolean))
        ).sort();
  ...
  const filteredItems =
    selectedPredictionProject &&
    selectedPredictionProject !== "No projects available"
      ? Array.from(
          new Set(
            requestsData
              .filter((r) => r.project_display === selectedPredictionProject)
              .map((r) => r.item_name)
              .filter(Boolean)
          )
        ).map((name) => ({ name, project: selectedPredictionProject }))
      : allItems;
  ...
  const handlePredict = async () => {
    if (selectedItem && selectedPredictionProject !== "all-projects") {
      try {
        ...
        const numeric =
          typeof prediction === "number" ? prediction : Number(prediction);
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <Select
                  value={selectedPredictionProject}
                  onValueChange={setSelectedPredictionProject}
                >
            ...
            {predictedAmount !== null && selectedItem ? (
              ...
              <div className="w-full p-6 rounded-xl border-2 border-gray-200 bg-gray-50 flex items-center justify-center text-gray-500">
                <div className="text-center">
                  ...
                  <p className="text-sm">
                    Select project and item to see prediction
                  </p>
                </div>
              </div>
            )}
